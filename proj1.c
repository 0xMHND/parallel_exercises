...
#pragma omp parallel for reduction(+:volume)
	for(int i = 0; i < NUMS*NUMS; i++ )
	   ...
	   {
	      if( (iv==iu)  || ((iu==0)&&(iv==NUMS-1)) || ((iu==NUMS-1)&&(iv==0)) ) 
		 volume = volume + (fullTileArea*Height(iu,iv)*0.25);
	      else
		 volume = volume + (fullTileArea*Height(iu,iv)*0.5);
	   }
	   else
	...
	printf("Threads:%d\tvolume=%f\tNUMS:%5d\ttime:%f\n", NUMT, volume, NUMS, time1-time0);	
	return 0;
}
