...
main( int argc, char *argv[ ] )
{
	...
	for( int x=0; x<NUMTRY; x++)
	{
		...
		double time2 = time1-time0;
		if(min_time > time2)
			min_time = time2;
		if(max_time < time2)
			max_time = time2;
		avg_time = avg_time+time2;
	}
	avg_time = (avg_time/NUMTRY);
	// print performance here:::
	if(FORC==0)
	{
		printf("coarse-grain: NUMT %d\n\t\tavg= %f sec,%f megaBodies/second"
			"\n\t\tmax= %f sec, %f megaBodies/second\n"
			"\t\tmin= %f sec, %f megaBodies/second\n",NUMT, avg_time, (float)(NUMBODIES*NUMBODIES*NUMSTEPS)/(avg_time)/1000000, max_time, (float)(NUMBODIES*NUMBODIES*NUMSTEPS)/(max_time)/1000000, min_time, (float)(NUMBODIES*NUMBODIES*NUMSTEPS)/(min_time)/1000000);
	}
	if(FORC==1)
	{
		printf("fine-grain: NUMT %d\n\t\tavg= %f sec,%f megaBodies/second"
			"\n\t\tmax= %f sec, %f megaBodies/second\n"
			"\t\tmin= %f sec, %f megaBodies/second\n",NUMT, avg_time, (float)(NUMBODIES*NUMBODIES*NUMSTEPS)/(avg_time)/1000000, max_time, (float)(NUMBODIES*NUMBODIES*NUMSTEPS)/(max_time)/1000000, min_time, (float)(NUMBODIES*NUMBODIES*NUMSTEPS)/(min_time)/1000000);
	}
	return 0;
